var Accordion = {
    ...
    $( "#accordion" ).mousemove(function( event ) {
        ...
        //loop through li's 
        var solveRate = function(finalPct, distance){
            // console.log(Math.log(finalPct));
            return (Math.log(finalPct)) / -(distance);
        };

        for (i=0; i<li.length; i++) {

            // console.log(parseInt($(li[i]).css("left").replace(/[^-\d\.]/g, ''),10) + 53 );

            if ( ( event.clientX > parseInt($(li[i]).css("left").replace(/[^-\d\.]/g, ''),10) ) && ( event.clientX < parseInt($(li[i]).css("left").replace(/[^-\d\.]/g, ''),10) + sectorSize ) )   {

               
                //loop through elements here
                $(li[i]).css("left", $(li[i]).position().x);
                $(li[i]).css("width", 200 );
                

                var distance = $("#accordion")[0].clientWidth - parseInt($(li[i]).css("left"),10);
                
                var rate = solveRate(0.2, distance);
                //console.log(rate);
                
                $(li[i]).css("width", -(parseInt($(li[i]).css("width").replace(/[^-\d\.]/g, ''),10) / (-(rate * parseInt($(li[i]).position().left,10)))));
                    // $(li[i]).css("width") / (-(rate * parseInt($(li[i]).position().left,10)))
                    // $(li[i]).css("width", $(li[i]).css("width") / (-(rate * $(li[i]).position().x)))
                
                // for (j=0; j<li.length; j++) {
                //     //chk logic here
                //     //for right
                //    if (event.clientX < parseInt($(li[j]).css("left").replace(/[^-\d\.]/g, ''),10) + sectorSize ) {
                //     $(li[j]).css("width", $(li[i]).css("width") / (-(rate * $(li[i]).css("left"))));

                //     //for left
                //    } else if (event.clientX > parseInt($(li[j]).css("left").replace(/[^-\d\.]/g, ''),10)) {
...
//             var distanceBetweenPx = Math.abs(event.clientX - $(li[i]).css("left").replace(/[^-\d\.]/g, ''));
...
// var singleWidth = totalHalfPx / (li.length/2);
...
//            // $(li[i]).find("p").text(x);
//             // $(li[i]).css("width", 200);
//             $(li[i]).css("width", singleWidth);
//             //console.log( $(li[i]).width() );
// // console.log(event.clientX);
// // console.log(($(li[i]).width())*li.length);
            

// // console.log($(li[i]).position().left);
//             if ( ( event.clientX > $(li[i]).position().left ) && ( event.clientX < ($(li[i]).position().left + 53) ) ) {

//                 $(li[i]).find("p").text("yup");
                
//                 j=i;
//                 k=1;
//                 while ($(li[i]).width() < 200) {
                    
//                     $(li[i]).width( $(li[i]).width()+20 );

//                     $(li[j-k]).width( $(li[j-k]).width()-10 );
//                     $(li[j+k]).width( $(li[j+k]).width()-10 );
//                     j=j+1;
//                     k=k+1;
//                 }
